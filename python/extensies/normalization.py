...
class ScaleNormalization(object):
    ...
    def normalise(self, x):
        ...
        return ((x - self.__min) / (self.__max - self.__min)) * self.__range_max + self.__range_min
...
class ZScoreNormalization(object):
    ...
    def fit(self, x):
        ...
        # Get the initial parameter for the given data
        self.__init_from_data__(x, self.__norm_factor)
        

        print('The parameters fitted are {}'.format(self.get_parameters()))
...
class GaussianNormalisation(object):
    ...
    def fit(self, x, x_range):
        ...
        x_pdf, x_pdf_bins = self.__compute_histogram__(x[x>0], x_range)
